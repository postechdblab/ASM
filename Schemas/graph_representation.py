...
class SchemaGraph:
    ...
    def get_FK_tables(self):
        FK_dict = {}
        PK_count = {}
        loaded_tables = {}

        for table in self.tables:
            if table.table_name not in loaded_tables:
                loaded_tables[table.table_name] = pd.read_csv(table.csv_file_location, low_memory=False, keep_default_na=False, na_values=[''])

        for rel in self.relationships:
            s_table = loaded_tables[rel.start]
            e_table = loaded_tables[rel.end]

            if s_table[rel.start_attr].nunique() < len(s_table):
                print(f'table {rel.start} key {rel.start_attr} # unique {s_table[rel.start_attr].nunique()} < {len(s_table)}')
                FK_dict[rel.start] = 1
                if rel.end in PK_count:
                    PK_count[rel.end] += 1
                else:
                    PK_count[rel.end] = 1
            if e_table[rel.end_attr].nunique() < len(e_table):
                print(f'table {rel.end} key {rel.end_attr} # unique {e_table[rel.end_attr].nunique()} < {len(e_table)}')
                FK_dict[rel.end] = 1
                if rel.start in PK_count:
                    PK_count[rel.start] += 1
                else:
                    PK_count[rel.start] = 1

        for t in FK_dict:
            print('FK table', t)
        print('total', len(FK_dict), 'FK tables out of', len(self.tables))
        for t in PK_count:
            print('PK table # reference', t, PK_count[t])
