...
class MaskedLinear(nn.Linear):
    ...
    def set_cached_mask(self, mask):
        assert False
        self.diag_mask.data.copy_(mask)
...
class MaskedResidualBlock(nn.Module):
    ...
    def set_diag_mask(self, mask):
        self.layers[0].set_diag_mask(mask)
        self.layers[1].set_diag_mask(mask)
...
class MADE(nn.Module):
    ...
    def __init__(
        ...
        assert input_encoding in [None, 'one_hot', 'embed', 'minmax']
        self.input_encoding = input_encoding
        assert output_encoding in [None, 'one_hot', 'embed', 'sigmoid']
        #if num_dmol > 0:
        ...
        self.subvar_dropout = subvar_dropout
        self.adjust_fact_col = adjust_fact_col
        ...
        print('input_bins', self.input_bins)
        print('nin', nin)
    ...
    def UseDMoL(self, natural_idx):
        #print('UseDMoL?', natural_idx, self.num_dmol, self.dmol_col_indexes)
    ...
    def _build_or_update_direct_io(self):
        ...
        for ord_i in range(self.nin):
            ...
            for ord_j in range(ord_i, self.nin):
                ...
                if ord_j == ord_i:
                    ...
                    mask[out_l:out_r, inp_l:inp_r] = 1 #target
    ...
    def _get_input_encoded_dist_size(self, dist_size, i):
        ...
        if self.input_encoding == 'embed':
            ...
            pass
        elif self.input_encoding == 'minmax':
            # use 2, 1 for value and 1 for sign
            return 2
        elif self.input_encoding is None:
            # it's the number of floats! not bits!
            return 1
        else:
    ...
    def update_masks(self, invoke_order=None):
        ...
        masks = None
        diag_masks = [None for l in range(L)]
        masks =      [self.m[l - 1][:, None] <= self.m[l][None, :] for l in range(L)]
        diag_masks.append(None)

        masks.append(self.m[L - 1][:, None] < self.m[-1][None, :])
        assert len(diag_masks) == len(masks) == L+1
        ...
        for i, (l, m, dm) in enumerate(zip(layers, masks, diag_masks)):
            l.set_mask(m)
            if dm is not None:
                l.set_diag_mask(dm)
    ...
    def Embed(self, data, natural_col=None, out=None):
        if debug:
            print(f'(Embed) data {data} idx {natural_col} out {out}')
        ...
        if natural_col is not None:
            ...
            if coli_dom_size > self.embed_size or not self.input_no_emb_if_leq:
                ...
                res = torch.zeros(bs, coli_dom_size, device=data.device)
                res.scatter_(1, data, 1)
                if out is not None:
                    out.copy_(res)
                    return out
                return res
            ...
            for i, coli_dom_size in enumerate(self.input_bins):
                assert torch.min(data[:,i]) >= 0
                assert torch.max(data[:,i]) < coli_dom_size, f'i = {i}, n = {len(self.input_bins)}, max = {torch.max(data[:,i])} < {coli_dom_size}'
                ...
                if coli_dom_size > self.embed_size or not self.input_no_emb_if_leq:
                    ...
                    if not self.dropout_p:
                        ...
                        if self.subvar_dropout and self.adjust_fact_col:
                            pass  # Use previous column's batch mask
                        else:
                            if self.subvar_dropout:
                                batch_mask = torch.zeros(bs, 1, device=data.device)
                            elif self.grouped_dropout:
                                pass  # Use previous column's batch mask
                    ...
                    if self.subvar_dropout and self.adjust_fact_col and coli_dom_size == 1:
                        y_onehot = dropped_repr
                    else:
    ...
    def logits_for_col(self, idx, logits, out=None):
        ...
        if embed is None:
            ...
            # logit_mem = logits_for_var.element_size() * logits_for_var.nelement()/(1024*1024)
            # print(f"{idx},{logit_mem:0.2f}MB,")
            return logits_for_var
    ...
    def nll(self, logits, data, label_smoothing=0, mask=None):
