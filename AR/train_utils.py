    ...
    for name, p in model.named_parameters():
        if blacklist is None or blacklist not in name:
            ...
            if p.grad is not None:
                grad_ps.append(np.prod(p.size()))

    num_params = sum(ps)
    num_grad_params = sum(grad_ps)
    mb = num_params * 4 / 1024 / 1024
    print('number of model parameters: {} (~= {:.1f}MB)'.format(num_params, mb))
    print('number of learnable model parameters: {} (~= {:.1f}MB)'.format(num_grad_params, num_grad_params * 4 / 1024 / 1024))
    print(model)
    return mb
